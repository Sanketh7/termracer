...
use crossterm::{execute, terminal, event, event::{Event, KeyCode}};
use views::view::{View, KeyEventHandleable};
...
fn main() {
    ...
    execute!(buf, terminal::EnterAlternateScreen).expect("ERROR: Failed to enter alternate screen.");
    ...
    execute!(buf, terminal::LeaveAlternateScreen).expect("ERROR: Failed to leave alternate screen.");
}
